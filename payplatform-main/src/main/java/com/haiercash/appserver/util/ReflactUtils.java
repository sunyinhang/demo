...
public class ReflactUtils {
    ...
    public static String getChannelNoByChannelAndChannelNo(String channel, String channelNo) {
        String retChannelNo = "";
        if ("16".equals(channel) || "31".equals(channelNo)) {//星巢贷
            retChannelNo = "31";
        }
        if ("13".equals(channel) || "14".equals(channel) || StringUtils.isEmpty(channel)) {//商户版13 个人版14 channel为空（老版本）
            retChannelNo = channelNo;
            if (StringUtils.isEmpty(channelNo)) {
                retChannelNo = "05";//默认
            }
        }
        if ("04".equals(channel)) {//嗨付默认 取channelNo
            retChannelNo = channelNo;
        }
        if ("34".equals(channelNo) || "35".equals(channelNo) || "33".equals(channelNo)) {//团大数据34 美分期35 豆子金融33
            retChannelNo = channelNo;
        }
        return retChannelNo;
    }
