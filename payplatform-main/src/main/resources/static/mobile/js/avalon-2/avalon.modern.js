...
/***/ function(module, exports, __webpack_require__) {
	...
	function initComponent(src, rawOption, local, template) {
		...
		if (avalon.type(definition.template) === 'function') {
			finalTemplate = definition.template.call(vmodel);
		} else {
			finalTemplate = definition.template.trim();
		}
...
/***/ function(module, exports) {
	...
	function fireDisposeHook(el) {
	    if (el.nodeType === 1 && el.getAttribute('wid') && !inDomTree(el)) {
	        ...
	        // by lihua begin
            if (avalon.scopes[vm.$id]) {
               delete avalon.scopes[vm.$id]
            }
            if (avalon.vmodels[vm.$id]) {
	...
	function parseExpr(binding) {
	    ...
	    if (category === 'on') {
	        ...
	        var setterBody = [
	            ...
	            'try{',
	            '\t' + body + ' = (__value__ === null || __value__ === undefined ? \'\' : __value__)',
	            '}catch(e){',
	        ...
	        var getterBody = [
	            ...
	            'var __value__ = ' + body,
	            filters.join('\n'),
	            'return (__value__ === null || __value__ === undefined ? \'\' : __value__)',
	            '}catch(e){',
