var page=1; //页数
var size=3; //条数
var url ='QueryLoanInfoServlet.do';
...
require(['avalon', 'jquery', 'util', 'Const', 'layer','iscroll','myiscroll','common'], function(avalon, $, util, Const) {	
	...
	var vm=avalon.define({
		$id: 'installmentBill1',
		arrList:[],
		appprogress: function(applSeq,outSts){
			util.cache({
				applSeq: applSeq
			});
			util.redirect("approvalProgress.html");
		},
		modiSubmit: function(applSeq,outSts){
			util.cache({
				applSeq: applSeq,
				back: '1', //表示审批退回的单子
			});
    			    		...
    			    		if(util.isEmpty(obj.retObj)){
    			    			$('#pullUp').html('已经是全部数据了，没有待加载的数据了。。。。');
        		       			...
        		       			for(var i=0;i<sizeTemp;i++){
        			        		...
        			        		else if(obj.retObj[i].outSts=="商户退回"||obj.retObj[i].outSts=="审批退回"){btnTextHtml='<a href="javascript:void(0)" onclick="appprogress(\''+obj.retObj[i].applSeq+'\',\''+obj.retObj[i].outSts+'\')" class="button payBlueBtn">审批进度</a><a href="javascript:void(0)" class="button payBlueBtn" id="modiSubmit" data-applseq="'+obj.retObj[i].applSeq+'">修改提交</a>';}
        			        		/*else if(obj.retObj[i].remainDays <0){btnTextHtml='<a href="javascript:void(0)" onclick="topay(\''+obj.retObj[i].applSeq+'\')" class="button payBlueBtn">还款</a>';}*/
        		        		    ...
        		        		    if(obj.retObj[i].remainDays){
        		        		    	 rehtml='<li class="iscrollAdd"><div class="billListD"><div class="timeTag"><span class="time">'+obj.retObj[i].applyDt+'</span><span class="tag">'+obj.retObj[i].outSts+'</span></div><div class="billInfoD" onclick="loanDetails(\''+num+'\',\''+fg+'\',\''+obj.retObj[i].outSts+'\')"><div class="div92"><div class="img"><img src="../mobile/themes/default/imgsb/installicon.png"/></div><div class="div92title"><span>'+title+'</span></div><div class="clear"></div></div></div><div class="installmentheji">'+i+'合计：&yen;<span class="blue028">'+obj.retObj[i].sybj+'</span></div><div class="botmLine"></div><div class="payBtnD">'+btnTextHtml+'<div class="clear"></div></div><div class="clear"></div></div></div></li>';	
        		        		    }
        		        		    else{
	...
	//修改提交
	$(document).on('click','#modiSubmit',function(){
		var applSeq=$(this).attr('data-applSeq');
		vm.modiSubmit(applSeq);
	})
