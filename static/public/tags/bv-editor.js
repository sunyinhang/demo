            ...
            editor.config.customUploadInit = function() {
                ...
                uploader.bind('FileUploaded', function (uploader, file, responseObject) {
                    ...
                    if (Const.rest.head) {
                        ...
                        urls.push(util.url(vm.viewUrl) + '/' + response.sign);
                    }
                });
                //全部文件上传时候
...
//                                console.log('即将插入图片' + value);
            ...
            editor.onchange = function () {
                // 编辑区域内容变化时，实时打印出当前内容
                vm.innerEntity[vm.name] = editor.$txt.html();
            };
            if (vm.innerEntity[vm.name]) {
                util.editor(editor, vm.innerEntity[vm.name]);
            }
        ...
        template: util.heredoc(function() {
            /*!
            <div :class="innerClass" :style="[{width: width}, innerStyle]">
                <textarea v-bind="innerAttr" :style="{height: height}"></textarea>
            </div>
            */
        })
    });
